...
namespace CostAllocationApp.DAL
{
    public class EmployeeAssignmentDAL : DbContext
    {
        ...
        {
            string where = "";
            where += $" ea.InChargeId in({inchargeIds}) and ";
        ...
        public List<ForecastDto> GetForecastCostData(int assignmentId, string year)
        {
            List<ForecastDto> forecasts = new List<ForecastDto>();
        ...
        *************/
        ...
        public List<ForecastAssignmentViewModel> GetCostByCompanyAndInchargeIds(string inchargeIds, string companyIds, int year, string timeStampsId)
        {
            string query = "";
            query = query + "SELECT ea.id as AssignmentId,emp.Id as EmployeeId,ea.EmployeeName,ea.SectionId, sec.Name as SectionName, ea.Remarks ";
            query = query + "    ,ea.ExplanationId,ea.DepartmentId, dep.Name as DepartmentName,ea.InChargeId, inc.Name as InchargeName ";
            query = query + "    ,ea.RoleId,rl.Name as RoleName,ea.CompanyId, com.Name as CompanyName, ea.UnitPrice ";
            query = query + "    ,gd.GradePoints,ea.IsActive,ea.GradeId,ea.BCYR,ea.BCYRCell,ea.IsActive,ea.BCYRApproved,ea.BCYRCellApproved ";
            query = query + "    ,ea.IsApproved,ea.BCYRCellPending,ea.IsRowPending,IsDeletePending,ea.EmployeeAssignmentId ";
            query = query + "FROM EmployeesAssignmentsWithTimeStamps ea left join Sections sec ON ea.SectionId = sec.Id ";
            query = query + "    LEFT JOIN Departments dep ON ea.DepartmentId = dep.Id ";
            query = query + "    LEFT JOIN Companies com ON ea.CompanyId = com.Id ";
            query = query + "    LEFT JOIN Roles rl ON ea.RoleId = rl.Id ";
            query = query + "    LEFT JOIN InCharges inc ON ea.InChargeId = inc.Id  ";
            query = query + "    LEFT JOIN Grades gd ON ea.GradeId = gd.Id ";
            query = query + "    LEFT JOIN Employees emp ON ea.EmployeeId = emp.Id ";
            query = query + "WHERE ea.TimeStampsId=" + timeStampsId + " AND ea.InChargeId in (" + inchargeIds + ") AND ea.CompanyId In (" + companyIds + ") AND ea.Year=" + year + " and  1=1 AND ea.IsDeleted is null OR  ea.IsDeleted=0 ";
            query = query + "ORDER BY emp.Id ASC ";
